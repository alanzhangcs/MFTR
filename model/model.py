...
class MLP_model(nn.Module):
    def __init__(self, gaze_encoder_out_dim, head_encoder_out_dim, pic_encoder_out_dim,dropout, seq_len, pred_len, pred_features, device):
        ...
        self.spatial_projection = nn.Linear(in_features=pic_encoder_out_dim, out_features=head_encoder_out_dim)


        self.spatial_temporal_embedding = TimeEmbedding(d_model=head_encoder_out_dim)
        self.spatial_temporal_norm = nn.LayerNorm(head_encoder_out_dim)
        self.spatial_temporal_attention = Attention([
                                    AttentionLayer(FullAttention(False, attention_dropout=dropout), d_model=head_encoder_out_dim,
                                                   n_heads=4) for i in range(temporal_attn_layers)
    ...
    def forward(self, head, gaze, pic):
        ...
        for i in range(pic.shape[0]):
            ...
            pic_featrues.append(feature.cpu().detach().numpy())

        pic_featrues = np.array(pic_featrues)
        pic_featrues = torch.from_numpy(pic_featrues)
        ...
        moving_feature = moving_feature + self.spatial_temporal_embedding(moving_feature)
        # print("pic features", pic_featrues.shape)
        # print("moving feartures", moving_feature.shape)
        features = pic_featrues + self.Dropout(self.spatial_temporal_attention(pic_featrues, moving_feature, moving_feature))



        # pred: batch x pre_len x pred_feature
